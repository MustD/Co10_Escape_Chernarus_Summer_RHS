...
params ["_position"];
private ["_object", "_marker", "_instanceNo", "_randomNo", "_gun", "_angle", "_car","_dir","_boxpos","_boxType"];

//Create a crashed object
_dir = random 360;
_object = createVehicle [a3e_arr_CrashSiteWrecks call BIS_fnc_selectRandom, _position, [], 0, "NONE"];
...
_boxpos = _position findEmptyPosition [3,15,_boxType];
 if(count _boxpos >0) then {
	...
	_deadcrew = _grp createUnit [a3e_arr_CrashSiteCrew call BIS_fnc_selectRandom, getpos _box, [], 15, "FORM"] ;
